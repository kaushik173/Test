...
function SaveData(buttonId) {
    ...
    if (($('#PlacementAgencyAddressID').val() > 0 && $('#ExistingAddressID').val() > 0) || ($('#PlacementAgencyAddressID').val() > 0 && ($('#Street').val().length > 0 || $('#City').val().length > 0 || $('#ZipCode').val().length > 0)) || ($('#ExistingAddressID').val() > 0 && ($('#Street').val().length > 0 || $('#City').val().length > 0 || $('#ZipCode').val().length > 0))) {
        ...
        Notify('Select an placement agency or select an existing address or enter new address information only', 'bottom-right', '4000', 'danger', 'fa-frown-o', true);
        return;
    }
    if ($('#Street').val().length > 0 && ($('#City').val().length == 0 || $('#ZipCode').val().length == 0 )) {
        Notify('Street,City,ZipCode and State is required!', 'bottom-right', '4000', 'danger', 'fa-frown-o', true);
        return;
        ...
        Notify('Street,City,ZipCode and State are required!', 'bottom-right', '4000', 'danger', 'fa-frown-o', true);
        return;
    }
    ...
    if ($('#PlacementAgencyAddressID').val() > 0 || $('#ExistingAddressID').val() > 0 || ($('#Street').val().length > 0 || $('#City').val().length > 0 || $('#ZipCode').val().length > 0 || $('#HomePhone').val().length > 0)) {
        ...
        $('.personRow').each(function () {
            if ($(this).find('select[id^="PersonAddressTypeCodeID"]').val() > 0) {
                typeSelected = true;
                if ($('#PlacementAgencyAddressID').val() > 0) {
                    if ($('#PlacementAgencyAddressID option:selected').data('id') == '25116' && $(this).find('select[id^="PersonAddressTypeCodeID"]').val()!='2448') {
                        $(this).find('input[id^="PersonAddressTypeCodeID"]').focus();
                        Notify('When Placement Agency/Institution is Absent Without Leave then Address Type must be AWOL.', 'bottom-right', '4000', 'danger', 'fa-frown-o', true);
                        errorReturn = true;
                        return;
                    }
                }
   ...
   /* var isPhoneNoExist = false;
    if ($('#HomePhone').val().trim() != "") {
        ...
        $.each(list, function (ind, obj) {
            ...
            if ($('#HomePhone').val().trim() == obj.trim()) {
                isPhoneNoExist = true;
                return false;
            }
    ...
    if (isPhoneNoExist) {
        Notify('Exact Home Phone found in Existing Addresses. Please verify this new address or select and existing address.', 'bottom-right', '4000', 'danger', 'fa-frown-o', true);
        $('#HomePhone').focus();
        return;
    }
    */
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Status == 'Done') {
                Notify('Data Saved!.', 'bottom-right', '3000', 'success', 'fa-smile-o', true);
                RequestSubmitted();
                fnRedirect(buttonId);
