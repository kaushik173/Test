...
function SaveData(buttonId) {
    ...
    if ($('#StateCodeID').val().length == 0) {

        Notify('State is required!', 'bottom-right', '4000', 'danger', 'fa-frown-o', true);
        return;
    }
    if (!hasFormChanged('case-addresses-form')) {
        if (Contains(document.location.href, 'dataentry=true'))
            document.location.href = '/CaseOpening/CaseAddresses?dataentry=true';
        else
